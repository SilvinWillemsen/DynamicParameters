...
ViolinString::ViolinString (double freq, double fs, int stringID, BowModel bowModelInit) : fs(fs), ogFreq(freq), stringID(stringID), bowModelInit (bowModelInit)
{
    ...
    for (int i = 0; i < 3; ++i)
        uVecs[i].resize(maxNumPoints);
    
    for (int i = 0; i < 2; ++i)
        uVecsTmp[i].resize(maxNumPoints);
...
//    _isBowing =  true;
    ...
    int loc = floor(0.5 * N);
    double width = floor(floor(N / 10.0) / 2.0) * 2.0;
    int startIdx = loc - width / 2;
    for (int l = startIdx; l < loc + width / 2; ++l)
    {
        u[l] = (1 - cos(2.0 * double_Pi * (l - startIdx) / width)) * 0.001;
        uPrev[l] = u[l];
        std::cout << uPrev[l] << std::endl;
    }
}
...
void ViolinString::updateGrid (double val)
{
    double cTemp = val;
    
    s1 = s1 / (rho * csA);
    double hTemp = sqrt((cTemp * cTemp * k * k + 4.0 * s1 * k + sqrt(pow(cTemp * cTemp * k * k + 4.0 * s1 * k, 2.0) + 16.0 * kappa * kappa * k * k)) * 0.5);
    
    s1 = s1 * rho * csA;
    int Ntemp = floor(L / hTemp); // Number of gridpoints
    hTemp = L / Ntemp; // Recalculate gridspacing
    
    if (Ntemp <= maxNumPoints)
    {
        fullGridInterpolation (Ntemp);
        c = cTemp;
        h = hTemp;
        N = Ntemp;
    }
...
void ViolinString::fullGridInterpolation (int Ngoal, bool smooth)
{
    double alphSpace = N / Ngoal;
    double alph = alphSpace;
    // Add the division h1/h2 division here somewhere from stefans book
    
    for (int n = 1; n < Ngoal; ++n)
    {
        uVecsTmp[0][n] = cubicInterpolation (u, n, alph);
        uVecsTmp[1][n] = cubicInterpolation (uPrev, n, alph);
        alph += alphSpace;
        if (alph >= 1)
        {
            alph -= 1;
    ...
    {
        u[n] = uVecsTmp[0][n];
        uPrev[n] = uVecsTmp[1][n];
    }
}

void ViolinString::analyseEnergy()
{
    kinEnergy = 0;
    potEnergy = 0;
    ...
    for (int l = 0; l < N; ++l)
    {
        kinEnergy = kinEnergy + rho * csA / 2.0 * h * 1.0 / (k * k) * (u[l] - uPrev[l]) * (u[l] - uPrev[l]);
        rOCkinEnergy = rOCkinEnergy + h * rho * csA / (2 * k * k * k) * (uNext[l] - 2 * u[l] + uPrev[l]) * (uNext[l] - uPrev[l]);
    }
    for (int l = 0; l < N-1; ++l)
    {
        potEnergy = potEnergy + T / 2.0 * 1.0 / h * (u[l+1] - u[l]) * (uPrev[l+1] - uPrev[l]);
    }
    for (int l = 1; l < N-1; ++l)
    {
        potEnergy = potEnergy + Eyoung * Iner / 2.0 * 10 / (h * h * h)
            * (u[l+1] - 2 * u[l] + u[l-1]) * (uPrev[l+1] - 2 * uPrev[l] + uPrev[l-1]);
        
        if (l == 1 && simplySupported)
            rOCpotEnergy = rOCpotEnergy + h * T / (2 * k * h * h) * (u[l+1] - 2 * u[l] + u[l-1]) * (uNext[l] - uPrev[l]) - h * Eyoung * Iner / (2 * k * h * h * h * h) * (u[l+2] - 4 * u[l+1] + 5 * u[l]) * (uNext[l] - uPrev[l]);
        else if (l == N - 2 && simplySupported)
            rOCpotEnergy = rOCpotEnergy + h * T / (2 * k * h * h) * (u[l+1] - 2 * u[l] + u[l-1]) * (uNext[l] - uPrev[l]) - h * Eyoung * Iner / (2 * k * h * h * h * h) * (5 * u[l] - 4 * u[l-1] + u[l+2]) * (uNext[l] - uPrev[l]);
        else
            rOCpotEnergy = rOCpotEnergy + h * T / (2 * k * h * h) * (u[l+1] - 2 * u[l] + u[l-1]) * (uNext[l] - uPrev[l]) - h * Eyoung * Iner / (2 * k * h * h * h * h) * (u[l+2] - 4 * u[l+1] + 6 * u[l] - 4 * u[l-1] + u[l+2]) * (uNext[l] - uPrev[l]);
        
        rOCdmpEnergy = rOCdmpEnergy - 2 * s0 * h / (4 * k * k) * (uNext[l] - uPrev[l]) * (uNext[l] - uPrev[l]);
        rOCdmpEnergy = rOCdmpEnergy + 2 * h * s1 / (2 * k * k * h * h) * (u[l+1] - 2 * u[l] + u[l-1] - (uPrev[l+1] - 2 * uPrev[l] + uPrev[l-1])) * (uNext[l] - uPrev[l]);
    }
    int bp = floor(bowPos.load());
    double alpha = bowPos.load() - bp;
    rOCbowEnergy = - (sig0 * z + sig1 * zDot + sig2 * q + sig3w) * (cubicInterpolation(uNext, bp, alpha) - cubicInterpolation(uPrev, bp, alpha)) / (2 * k);
    energy = kinEnergy + potEnergy;
    rOCenergy = rOCkinEnergy - rOCpotEnergy - rOCdmpEnergy - rOCbowEnergy;

    std::cout << rOCenergy << std::endl;
}
